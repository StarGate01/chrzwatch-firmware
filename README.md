...
Next, compile and upload the firmware using the `i6hrc_flashdump` environment. Then, launch the script `tools/flash.dump.py`. The content of the flash will be downloaded to `dump.bin`. However, on some models I noticed the flash memory to be empty.
